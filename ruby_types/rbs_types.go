...
func RbsModules(file pgs.File) []string {
	p := RbsPackage(file)
...
func RbsPackage(file pgs.File) string {
...
func RbsMessageType(entity EntityWithParent) string {
	names := make([]string, 0)
	outer := entity
	ok := true
	for ok {
	...
	return fmt.Sprintf("%s::%s", RbsPackage(entity.File()), strings.Join(names, "::"))
}
...
func RbsFieldValue(field pgs.Field) string {
	t := field.Type()
	if t.IsMap() {
		key := rbsMapType(t.Key())
		if t.Element().ProtoType() == pgs.MessageT {
			value := RbsMessageType(t.Element().Embed())
			return fmt.Sprintf("Google::Protobuf::Map.new(%s, :message, %s)", key, value)
		}
		value := rbsMapType(t.Element())
		return fmt.Sprintf("Google::Protobuf::Map.new(%s, %s)", key, value)
	} else if t.IsRepeated() {
		return "[]"
	}
	return rbsProtoTypeValue(field, t)
}
...
func rbsProtoTypeElem(field pgs.Field, ft FieldType, mt methodType) string {
	...
	if pt == pgs.MessageT {
		return fmt.Sprintf("T.nilable(%s)", RbsMessageType(ft.Embed()))
	}
	log.Panicf("Unsupported field type for field: %v\n", field.Name().String())
	return ""
}

func rbsProtoTypeValue(field pgs.Field, ft FieldType) string {
	pt := ft.ProtoType()
	if pt.IsInt() {
		return "0"
	}
...
func rbsMapType(ft FieldType) string {
	switch ft.ProtoType() {
	case pgs.DoubleT:
		return ":double"
	case pgs.FloatT:
